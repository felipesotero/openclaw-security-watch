...
function normalizePriority(priority) {
  const input = Array.isArray(priority) ? priority.filter((kind) => HUMAN_KINDS.includes(kind)) : [];
...
export function findHumanChannel({ agentId, config, priority }) {
  ...
  const enabledBindings = bindings.filter((binding) => binding && binding.enabled !== false && HUMAN_KINDS.includes(binding.kind));
  for (const kind of Array.isArray(priority) ? priority : HUMAN_KINDS) {
    const hit = enabledBindings.find((binding) => binding.kind === kind);
    if (hit) {
      const channelConfig = config?.channels?.[kind]?.[hit.id] ?? {};
      return { kind, id: hit.id, target: hit.target ?? channelConfig.target ?? channelConfig.handle ?? null };
    }
  }
  return null;
}
