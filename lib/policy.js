...
import { canonicalizePath, isInsideDirectory, nearestExistingAncestorRealpath, workspaceTrusted } from "./paths.js";
...
export function loadPolicy(pluginConfig = {}) {
  ...
  return {
    ...
    trustedWorkspacePrefixes: (base.trustedWorkspacePrefixes || []).map((prefix) => canonicalizePath(prefix)),
...
export function workspaceOperationalWriteAllowed(filePath, workspaceDir, policy, opts = {}) {
  const patterns = policy?.writeAllow?.workspaceSubdirPatterns;
  if (!Array.isArray(patterns) || patterns.length === 0) return false;

  const normalized = canonicalizePath(filePath, opts);
  if (!normalized || !workspaceDir) return false;

  const rel = path.relative(workspaceDir, normalized);
  if (!rel || rel.startsWith("..") || path.isAbsolute(rel)) return false;

  const workspaceRoot = canonicalizePath(workspaceDir, opts);
  const ancestorRealpath = nearestExistingAncestorRealpath(normalized);
  if (!workspaceRoot || !ancestorRealpath) return false;
  if (!isInsideDirectory(ancestorRealpath, workspaceRoot)) return false;

  return patterns.some((pattern) => new RegExp(pattern).test(rel));
}
...
export function evaluateToolCall({ toolName, params }, policy, context = {}) {
  ...
  const subject = toolName === "read" || toolName === "write" || toolName === "edit"
    ? canonicalizePath(rawSubject, { baseDir: workspaceDir })
    : rawSubject;
  const eventBase = {
    toolName,
    subject,
  ...
  if (toolName === "read" || toolName === "write" || toolName === "edit") {
    ...
    // Per-agent isolation: when workspaceDir is resolved, trust ONLY that workspace.
    // Fallback to global trustedWorkspacePrefixes only when workspace cannot be resolved.
    const trustedRoots = workspaceDir
      ? [canonicalizePath(workspaceDir)]
      : policy.trustedWorkspacePrefixes;
    const isTrustedWorkspace = workspaceTrusted(subject, trustedRoots, { baseDir: workspaceDir });
    if (toolName === "read" && isTrustedWorkspace) {
      return { outcome: "allow", severity: "info", reasons: ["read_allow:trusted_workspace"], ...eventBase };
    }
    if ((toolName === "write" || toolName === "edit") && isTrustedWorkspace && workspaceOperationalWriteAllowed(subject, workspaceDir, policy, { baseDir: workspaceDir })) {
