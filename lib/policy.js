...
function nearestExistingAncestorRealpath(filePath) {
...
export function workspaceOperationalWriteAllowed(filePath, workspaceDir, opts = {}) {
  ...
  const workspaceRoot = normalizePolicyPath(workspaceDir, opts);
  const ancestorRealpath = nearestExistingAncestorRealpath(normalized);
  if (!workspaceRoot || !ancestorRealpath) return false;
  if (!(ancestorRealpath === workspaceRoot || ancestorRealpath.startsWith(`${workspaceRoot}${path.sep}`))) return false;
