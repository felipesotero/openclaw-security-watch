...
export function workspaceOperationalWriteAllowed(filePath, workspaceDir, opts = {}) {
  const normalized = canonicalizePath(filePath, opts);
...
export function evaluateToolCall({ toolName, params }, policy, context = {}) {
  ...
  if (toolName === "read" || toolName === "write" || toolName === "edit") {
    ...
    if ((toolName === "write" || toolName === "edit") && isTrustedWorkspace && workspaceOperationalWriteAllowed(subject, workspaceDir, { baseDir: workspaceDir })) {
      return { outcome: "allow", severity: "info", reasons: ["write_allow:trusted_workspace_operational_output"], ...eventBase };
    }
