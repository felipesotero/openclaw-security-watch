...
export function normalizePolicyPath(rawPath) {
  ...
  if (!path.isAbsolute(normalized)) normalized = path.resolve(normalized);
...
export function loadPolicy(pluginConfig = {}) {
  ...
  return {
    ...
    approvalTimeoutMs: Number(pluginConfig.approvalTimeoutMs || 300000),
...
export function isTrustedWorkspaceRelativePath(rawPath) {
  const expanded = expandHome(safeString(rawPath));
...
export function evaluateToolCall({ toolName, params }, policy, context = {}) {
  ...
  const isAutomation = context.isAutomation === true && context.jobId;
  ...
  if (toolName === "read" || toolName === "write" || toolName === "edit") {
    ...
    const isTrustedWorkspace = workspaceTrusted(subject, policy.trustedWorkspacePrefixes);
    if (toolName === "read" && isTrustedWorkspace) {
      return { outcome: "allow", severity: "info", reasons: ["read_allow:trusted_workspace"], ...eventBase };
    }
    if (toolName === "read" && isTrustedWorkspaceRelativePath(rawSubject)) {
      return { outcome: "allow", severity: "info", reasons: ["read_allow:trusted_workspace_relative"], ...eventBase };
    }
