...
export function createBootstrapGrants({ jobId, agentId, paths = [], writes = [], edits = [], commands = [], urls = [], baseDir }) {
  ...
  return [
    ...paths.flatMap((subject) => {
      const subjectPattern = subjectPatternForPath(subject, opts);
    ...
    ...commands.flatMap((subject) => ["bash", "exec"].map((tool) => ({
      id: crypto.randomUUID(),
      jobId,
      agentId,
      toolName: tool,
